    ...
    <form action="{{route('user.post-comment.update',$comment->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($comment->status=='active')? 'selected' : '')}}>{{__("Active")}}</option>
          <option value="inactive" {{(($comment->status=='inactive')? 'selected' : '')}}>{{__("Inactive")}}</option>
        </select>
      </div>
