...
<div class="card shadow mb-4">
   ...
   <div class="card-header py-3">
     <h4 class=" font-weight-bold">{{__("Profile")}}</h4>
     <ul class="breadcrumbs">
         <li><a href="{{route('admin')}}" style="color:#999">{{__("Dashboard")}}</a></li>
         <li><a href="" class="active text-primary">{{__("Profile Page")}}</a></li>
     </ul>
   </div>
   <div class="card-body">
        <div class="row">
            ...
            <div class="col-md-8">
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                        <label for="inputTitle" class="col-form-label">{{__("Name")}}</label>
                      ...
                      <div class="form-group">
                          ...
                          <select name="role" class="form-control">
                              <option value="">-----{{__("Select Role")}}-----</option>
                                  <option value="admin" {{(($profile->role=='admin')? 'selected' : '')}}>{{__("Admin")}}</option>
                                  <option value="user" {{(($profile->role=='user')? 'selected' : '')}}>{{__("User")}}</option>
                          </select>
                        @error('role')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
