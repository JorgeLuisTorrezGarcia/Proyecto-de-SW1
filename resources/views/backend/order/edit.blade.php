                ...
                <div class="form-group">
                    ...
                    <select name="status" id="" class="form-control">
                        <option value="new"
                            ...
                            {{ $order->status == 'new' ? 'selected' : '' }}>{{ __('New') }}</option>
                        <option value="process"
                            {{ $order->status == 'delivered' || $order->status == 'cancel' ? 'disabled' : '' }}
                            {{ $order->status == 'process' ? 'selected' : '' }}>{{ __('process') }}</option>
                        <option value="delivered" {{ $order->status == 'cancel' ? 'disabled' : '' }}
                            {{ $order->status == 'delivered' ? 'selected' : '' }}>{{ __('Delivered') }}</option>
                        <option value="cancel" {{ $order->status == 'delivered' ? 'disabled' : '' }}
                            {{ $order->status == 'cancel' ? 'selected' : '' }}>{{ __('Cancel') }}</option>
                    </select>
                </div>
